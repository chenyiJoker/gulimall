package com.cy.gulimall.seckill.service.impl;
...
import com.cy.gulimall.seckill.feign.ProductFeignService;
import com.cy.gulimall.seckill.interceptor.LoginUserInterceptor;
import com.cy.gulimall.seckill.feign.CouponFeignService;
import com.cy.gulimall.seckill.service.SeckillService;
import com.cy.gulimall.seckill.to.SeckillSkuRedisTo;
import com.cy.gulimall.seckill.vo.SeckillSessionWithSkusVo;
import com.cy.gulimall.seckill.vo.SkuInfoVo;
        ...
        if (keys != null) {
            for (String key : keys) {
                ...
                if (currentTime >= startTime && currentTime <= endTime) {
                    ...
                    if (range != null) {
                        listValue = hasOps.multiGet(range);
                    }

                    if (listValue != null) {
                        List<SeckillSkuRedisTo> collect = listValue.stream()
                                .map(item -> JSON.parseObject(item, SeckillSkuRedisTo.class)).collect(Collectors.toList());
                        return collect;
                    }
                    break;
                }
